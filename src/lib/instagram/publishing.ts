...
export async function publishInstagramJob(jobId: string) {
  ...
  try {
    ...
    if (job.type === "POST") {
      if (media.length !== 1) {
        throw new Error("پست معمولی فقط باید یک فایل داشته باشد.");
      }

      if (media[0].type !== "IMAGE") {
        throw new Error("POST فعلاً فقط از تصویر پشتیبانی می‌کند.");
      }
      ...
      if (media.length !== 1) {
        throw new Error("Reel فقط باید یک فایل داشته باشد.");
      }

      if (media[0].type !== "VIDEO") {
        throw new Error("Reel باید فایل ویدیویی داشته باشد.");
      }
