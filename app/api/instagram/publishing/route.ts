  ...
  try {
    ...
    const jobs = await prisma.instagramPublishJob.findMany({
      ...
      include: {
        ...
        instagramAccount: { select: { id: true, igUsername: true, igUserId: true } },
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (data.type === "POST" && (data.media.length !== 1 || data.media[0].type !== "IMAGE")) {
      return NextResponse.json({ success: false, message: "POST باید دقیقاً یک تصویر داشته باشد." }, { status: 400 });
    }

    if (data.type === "REEL" && (data.media.length !== 1 || data.media[0].type !== "VIDEO")) {
      return NextResponse.json({ success: false, message: "REEL باید دقیقاً یک ویدیو داشته باشد." }, { status: 400 });
    }

    if (data.type === "STORY" && (data.media.length !== 1 || !["IMAGE", "VIDEO"].includes(data.media[0].type))) {
      return NextResponse.json({ success: false, message: "Story باید دقیقاً یک تصویر یا ویدیو داشته باشد." }, { status: 400 });
    }

    if (data.type === "CAROUSEL" && (data.media.length < 2 || data.media.length > 10 || data.media.some((item) => item.type !== "IMAGE"))) {
      return NextResponse.json({ success: false, message: "Carousel باید بین ۲ تا ۱۰ تصویر داشته باشد." }, { status: 400 });
    }
    ...
    if (scheduledAt && Number.isNaN(scheduledAt.getTime())) {
      return NextResponse.json({ success: false, message: "تاریخ زمان‌بندی معتبر نیست." }, { status: 400 });
    }
    ...
    try {
      ...
      if (error && typeof error === "object" && "code" in error && error.code === "P2002") {
        return NextResponse.json({ success: false, message: "این اکانت در همین تاریخ و ساعت از قبل یک محتوای زمان‌بندی‌شده دارد. زمان دیگری انتخاب کنید." }, { status: 409 });
      }
      throw error;
    }
    ...
    if (isScheduled && scheduledAt) {
      try {
        ...
        await prisma.instagramPublishJob.update({
          where: { id: job.id },
          data: { status: "FAILED", errorMessage: error instanceof Error ? `شروع Workflow ناموفق بود: ${error.message}` : "شروع Workflow ناموفق بود.", retryCount: { increment: 1 } },
        });
        throw error;
      }
    }
