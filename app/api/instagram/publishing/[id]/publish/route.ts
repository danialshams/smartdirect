...
export async function POST(_request: NextRequest, context: Context) {
  try {
    ...
    if (["PUBLISHED", "PUBLISHING", "PROCESSING"].includes(job.status)) {
      return NextResponse.json({ success: false, message: "این Job در حال انتشار یا قبلاً منتشر شده است." }, { status: 409 });
    }
    ...
    return NextResponse.json({
      success: false,
      message: error instanceof Error ? error.message : "خطای داخلی هنگام انتشار.",
    }, { status: 500 });
  }
}
