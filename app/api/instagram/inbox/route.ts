...
async function getOwnedAccount(userId: string, accountId?: string | null) {
    return prisma.instagramAccount.findFirst({
        where: { userId, ...(accountId ? { id: accountId } : {}), isConnected: true },
...
async function enrichParticipantProfile(accountId: string, participantId: string, accessToken: string) {
    try {
        const response = await fetch(`${GRAPH_BASE}/${encodeURIComponent(participantId)}?fields=name,username,profile_pic`, {
            headers: { Authorization: `Bearer ${accessToken}` },
            cache: "no-store",
        });
        if (!response.ok) return;
        const profile = (await response.json()) as { name?: string; username?: string; profile_pic?: string };
        ...
        await prisma.conversation.updateMany({
            ...
            data: {
                ...
                ...(profile.profile_pic ? { participantProfilePicture: profile.profile_pic } : {}),
            },
        });
    } catch (error) {
...
async function uploadInstagramAttachment({ igUserId, accessToken, file }: { igUserId: string; accessToken: string; file: File }) {
    const mimeType = file.type.toLowerCase();
    const attachmentType = mimeType.startsWith("image/") ? "image" : mimeType.startsWith("video/") ? "video" : mimeType.startsWith("audio/") ? "audio" : null;
    if (!attachmentType) throw new Error("فقط فایل‌های عکس، ویدیو و صوت قابل ارسال هستند.");
    if (file.size > 25 * 1024 * 1024) throw new Error("حجم فایل نمی‌تواند بیشتر از ۲۵ مگابایت باشد.");

    const uploadBody = new FormData();
    uploadBody.append("message", JSON.stringify({ attachment: { type: attachmentType, payload: { is_reusable: false } } }));
    uploadBody.append("filedata", new Blob([await file.arrayBuffer()], { type: mimeType }), "smartdirect-media");

    const response = await fetch(`${GRAPH_BASE}/${igUserId}/message_attachments`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}` },
        body: uploadBody,
        cache: "no-store",
    });
    const data = (await response.json().catch(() => ({}))) as { attachment_id?: string; error?: { message?: string } };
    if (!response.ok || !data.attachment_id) throw new Error(data.error?.message || "آپلود فایل به Instagram ناموفق بود.");
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (conversationId) {
            const conversation = await prisma.conversation.findFirst({
                ...
                include: {
                    messages: {
                        ...
                        select: { id: true, direction: true, messageType: true, text: true, mediaUrl: true, mediaId: true, igMessageId: true, readAt: true, seenAt: true, createdAt: true },
                    },
                },
            });
            ...
            if (!conversation.participantUsername) await enrichParticipantProfile(account.id, conversation.participantId, accessToken);
        ...
        await Promise.all(
            conversations.filter((item) => !item.participantUsername).slice(0, 25).map((item) => enrichParticipantProfile(account.id, item.participantId, accessToken)),
        );

        const unread = await prisma.conversationMessage.groupBy({
            by: ["conversationId"],
            where: { conversationId: { in: conversations.map((item) => item.id) }, direction: "INBOUND", readAt: null },
            _count: { _all: true },
        });
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (contentType.includes("multipart/form-data")) {
            ...
            body = { accountId: form.get("accountId"), conversationId: form.get("conversationId"), text: form.get("text"), replyToMessageId: form.get("replyToMessageId") };
        ...
        if (!accountId || !conversationId) return jsonError("accountId و conversationId الزامی هستند.");
        ...
        const messageBody: Record<string, unknown> = { recipient: { id: conversation.participantId } };

        if (file) {
            const uploaded = await uploadInstagramAttachment({ igUserId: account.igUserId, accessToken, file });
            mediaId = uploaded.attachmentId;
            messageType = uploaded.attachmentType === "image" ? "IMAGE" : uploaded.attachmentType === "video" ? "VIDEO" : "AUDIO";
            messageBody.message = { attachment: { type: uploaded.attachmentType, payload: { attachment_id: uploaded.attachmentId } } };
        } else {
        ...
        if (replyToMessageId) {
            ...
            if (replyTarget?.igMessageId) messageBody.reply_to = { mid: replyTarget.igMessageId };
        }
        ...
        if (!response.ok) return jsonError(data.error?.message || "Instagram پیام را ارسال نکرد.", response.status >= 400 && response.status < 500 ? response.status : 502);

        const createdMessage = await prisma.conversationMessage.create({
            data: { conversationId: conversation.id, direction: "OUTBOUND", messageType, text: text || null, mediaUrl, mediaId, igMessageId: data.message_id || undefined },
            select: { id: true, direction: true, messageType: true, text: true, mediaUrl: true, mediaId: true, igMessageId: true, readAt: true, seenAt: true, createdAt: true },
        });
