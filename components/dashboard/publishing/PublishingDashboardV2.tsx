...
function PersianDatePicker({ value, onChange }: { value: JalaliDate; onChange: (value: JalaliDate) => void }) {
  ...
  return <div className="relative"><button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center justify-between rounded-lg border bg-background px-4 py-3 text-right text-sm hover:border-slate-300"><CalendarClock size={18} className="text-muted-foreground" /><span className="font-medium text-foreground">{jalaliMonths[value.month - 1]} {toPersianDigits(value.day)}، {toPersianDigits(value.year)}</span></button>{open && <div className="absolute right-0 z-30 mt-2 w-full min-w-[300px] rounded-xl border bg-card p-4 shadow-xl"><div className="mb-4 flex items-center justify-between"><button type="button" onClick={() => moveMonth(1)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><ChevronRight size={18} /></button><div className="text-sm font-bold text-foreground">{jalaliMonths[view.month - 1]} {toPersianDigits(view.year)}</div><button type="button" onClick={() => moveMonth(-1)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><ChevronLeft size={18} /></button></div><div className="mb-2 grid grid-cols-7 gap-1 text-center text-xs font-medium text-muted-foreground">{weekDays.map((day) => <span key={day}>{day}</span>)}</div><div className="grid grid-cols-7 gap-1">{Array.from({ length: firstWeekday }).map((_, i) => <span key={`empty-${i}`} />)}{Array.from({ length: days }, (_, i) => i + 1).map((day) => <button key={day} type="button" onClick={() => { onChange({ year: view.year, month: view.month, day }); setOpen(false); }} className={["aspect-square rounded-lg text-sm", value.year === view.year && value.month === view.month && value.day === day ? "bg-slate-950 text-white" : "text-foreground hover:bg-muted"].join(" ")}>{toPersianDigits(day)}</button>)}</div></div>}</div>;
}
