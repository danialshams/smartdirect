      ...
      <section className="mx-auto w-full max-w-6xl rounded-xl border bg-card p-5 shadow-sm">
        <div className="flex flex-col gap-4">
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_120px_120px_auto]">
            <input value={draft.username} onChange={(event) => setDraft((current) => ({ ...current, username: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTag(); } }} placeholder="username یا @username" className="h-11 rounded-xl border border-border px-3 text-sm outline-none transition focus:border-slate-400" dir="ltr" />
            <input value={draft.x} onChange={(event) => setDraft((current) => ({ ...current, x: Number(event.target.value) }))} type="number" min="0" max="1" step="0.01" placeholder="X" className="h-11 rounded-xl border border-border px-3 text-sm outline-none transition focus:border-slate-400" dir="ltr" aria-label="X position" />
            <input value={draft.y} onChange={(event) => setDraft((current) => ({ ...current, y: Number(event.target.value) }))} type="number" min="0" max="1" step="0.01" placeholder="Y" className="h-11 rounded-xl border border-border px-3 text-sm outline-none transition focus:border-slate-400" dir="ltr" aria-label="Y position" />
