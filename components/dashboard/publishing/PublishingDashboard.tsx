...
function gregorianToJalali(gy: number, gm: number, gd: number): JalaliDate {
  ...
  if (new Date(gy, gm - 1, gd).getTime() < new Date(candidate[0], candidate[1] - 1, candidate[2]).getTime()) {
    jy -= 1;
  }

  const start = jalaliToGregorian(jy, 1, 1);
  const diff = Math.floor(
    (new Date(gy, gm - 1, gd).getTime() - new Date(start[0], start[1] - 1, start[2]).getTime()) /
      86400000,
  );
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 text-right text-sm outline-none transition hover:border-slate-300"
      >
        <CalendarClock size={18} className="text-slate-500" />
        <span className="font-medium text-slate-800">
          {jalaliMonths[value.month - 1]} {toPersianDigits(value.day)}، {toPersianDigits(value.year)}
        </span>
      </button>
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-full min-w-[300px] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
          <div className="mb-4 flex items-center justify-between">
            <button type="button" onClick={() => moveMonth(1)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100">
              <ChevronRight size={18} />
            </button>
            <div className="text-sm font-bold text-slate-900">
              {jalaliMonths[view.month - 1]} {toPersianDigits(view.year)}
            </div>
            <button type="button" onClick={() => moveMonth(-1)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100">
              <ChevronLeft size={18} />
            </button>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: days }, (_, index) => index + 1).map((day) => {
              const selected = value.year === view.year && value.month === view.month && value.day === day;
              return (
                <button
                  key={day}
                  type="button"
...
function uploadFileWithProgress(file: File, onProgress: (value: number) => void) {
  return new Promise<UploadedMedia>((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error(`آپلود ${file.name} ناموفق بود.`));
...
export default function PublishingDashboard() {
  ...
  const [minute, setMinute] = useState(Math.ceil(new Date().getMinutes() / 5) * 5 >= 60 ? 0 : Math.ceil(new Date().getMinutes() / 5) * 5);
  ...
  function revokeLocalMedia(items: LocalMedia[]) {
  ...
  function handleFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const remaining = Math.max(0, 10 - media.length - uploadedMedia.length);
    const selected = type === "CAROUSEL" ? accepted.slice(0, remaining) : accepted.slice(0, 1);
    ...
    setMedia((current) => [
      ...
      ...selected.map((file, index) => ({
        ...
        sortOrder: uploadedMedia.length + current.length + index,
      })),
    ]);
  }
  ...
  function removeLocal(index: number) {
    ...
    setMedia((current) => current.filter((_, itemIndex) => itemIndex !== index).map((item, itemIndex) => ({ ...item, sortOrder: itemIndex + uploadedMedia.length })));
  }
  ...
  async function removeUploaded(item: UploadedMedia) {
    try {
      ...
      setUploadedMedia((current) => current.filter((mediaItem) => mediaItem.storageKey !== item.storageKey).map((mediaItem, index) => ({ ...mediaItem, sortOrder: index })));
    } catch (removeError) {
      setError(removeError instanceof Error ? removeError.message : "حذف فایل ناموفق بود.");
    }
  }
  ...
  async function uploadSelectedMedia() {
    ...
    if (type === "CAROUSEL" && media.length + uploadedMedia.length < 2) {
    ...
    try {
      ...
      for (let index = 0; index < media.length; index += 1) {
        ...
        const result = await uploadFileWithProgress(item.file, (fileProgress) => {
          const currentBytes = completedBytes + (item.file.size * fileProgress) / 100;
          setUploadProgress(totalBytes ? Math.min(100, Math.round((currentBytes / totalBytes) * 100)) : fileProgress);
        });
        results.push({ ...result, sortOrder: uploadedMedia.length + results.length });
  ...
  async function createJob(publishNow: boolean) {
    ...
    if (!uploadedMedia.length) {
      setError("ابتدا فایل را آپلود کنید.");
      return;
    }
    if (type === "CAROUSEL" && uploadedMedia.length < 2) {
      setError("Carousel باید حداقل دو تصویر داشته باشد.");
      return;
    }
    ...
    if (!publishNow && scheduled.getTime() <= Date.now()) {
      setError("زمان‌بندی باید در آینده باشد.");
      return;
    }
    ...
    try {
      ...
      setUploadedMedia([]);
      setUploadProgress(0);
      setScheduledDate(currentJalaliDate());
      await loadJobs();
    } catch (publishError) {
  ...
  const canSchedule = uploadedMedia.length > 0 && !uploading && !publishing;
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-400">ابتدا فایل را آپلود کنید؛ بعد از تکمیل آپلود، تنظیم زمان انتشار فعال می‌شود.</p>
            ...
            <div className="my-6 grid grid-cols-3 gap-2">
              {([
                ...
                <button key={value} type="button" onClick={() => { clearLocalMedia(); setUploadedMedia([]); setType(value); setUploadProgress(0); }} className={["flex flex-col items-center justify-center gap-2 rounded-xl border px-3 py-4 text-sm transition", type === value ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"].join(" ")}>
                  <Icon size={20} />{label}
                </button>
              ))}
            </div>
            ...
            <div className="mb-5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
              <label className="block cursor-pointer">
                <span className="mb-2 block text-sm font-medium text-slate-700">
                  {type === "CAROUSEL" ? "تصاویر Carousel" : type === "REEL" ? "ویدیوی Reel" : "تصویر پست"}
                </span>
                <input type="file" accept={type === "REEL" ? "video/mp4,video/quicktime" : "image/jpeg,image/png,image/webp"} multiple={type === "CAROUSEL"} onChange={handleFiles} disabled={uploading || publishing || (type === "CAROUSEL" && uploadedMedia.length >= 10)} className="block w-full cursor-pointer text-sm" />
              </label>
              <p className="mt-2 text-xs text-slate-400">
                {type === "CAROUSEL" ? `حداکثر ۱۰ تصویر — ${toPersianDigits(uploadedMedia.length + media.length)} فایل انتخاب/آپلود شده` : "فایل پس از انتخاب هنوز به Storage ارسال نشده است."}
              </p>
            </div>
            ...
            {(media.length > 0 || uploadedMedia.length > 0) && (
              <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                {uploadedMedia.map((item) => (
                  <div key={item.storageKey} className="relative overflow-hidden rounded-xl border border-slate-200 bg-white">
                    ...
                    <button type="button" onClick={() => void removeUploaded(item)} disabled={uploading || publishing} className="absolute left-2 top-2 rounded-full bg-white/95 p-1.5 text-red-600 shadow-sm hover:bg-red-50 disabled:opacity-50" aria-label="حذف فایل">
                      <X size={15} />
                    </button>
                    <div className="flex items-center justify-between gap-2 px-2 py-2 text-xs">
                      <span className="truncate text-slate-600">{item.fileName}</span>
                ...
                {media.map((item, index) => (
                  <div key={`${item.file.name}-${item.sortOrder}`} className="relative overflow-hidden rounded-xl border border-dashed border-slate-300 bg-slate-50">
                    ...
                    <button type="button" onClick={() => removeLocal(index)} disabled={uploading || publishing} className="absolute left-2 top-2 rounded-full bg-white/95 p-1.5 text-red-600 shadow-sm hover:bg-red-50 disabled:opacity-50" aria-label="حذف فایل انتخاب‌شده">
                      <X size={15} />
                    </button>
                    <div className="truncate px-2 py-2 text-xs text-slate-500">در انتظار آپلود</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {uploading && (
              <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4">
                <div className="mb-2 flex items-center justify-between text-sm">
                  <span className="font-medium text-slate-700">در حال آپلود محتوا</span>
                  <span className="font-bold text-slate-950">{toPersianDigits(uploadProgress)}٪</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full rounded-full bg-slate-950 transition-all duration-200" style={{ width: `${uploadProgress}%` }} />
                </div>
                <p className="mt-2 text-xs text-slate-400">
                  فایل {toPersianDigits(uploadIndex)} از {toPersianDigits(media.length)} — بعد از رسیدن به ۱۰۰٪، مرحله تنظیم انتشار فعال می‌شود.
                </p>
              </div>
            )}
            ...
            {!uploading && uploadedMedia.length > 0 && (
              <div className="mb-5 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                ...
                <span>آپلود محتوا با موفقیت کامل شد. حالا می‌توانید زمان انتشار را تنظیم کنید.</span>
              </div>
            )}

            {uploadedMedia.length === 0 && media.length > 0 && (
              <button type="button" onClick={() => void uploadSelectedMedia()} disabled={uploading || publishing || loading} className="mb-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <>
                ...
                <div className="mb-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div>
                      <span className="block text-sm font-medium text-slate-700">زمان‌بندی انتشار</span>
                      <span className="mt-1 block text-xs text-slate-400">تقویم کاملاً شمسی</span>
                    </div>
                    <CalendarClock size={18} className="text-slate-500" />
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <label>
                      <span className="mb-1 block text-xs text-slate-500">ساعت</span>
                      <select value={hour} onChange={(event) => setHour(Number(event.target.value))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm">
                        {Array.from({ length: 24 }, (_, value) => <option key={value} value={value}>{toPersianDigits(String(value).padStart(2, "0"))}</option>)}
                      </select>
                    </label>
                    <label>
                      <span className="mb-1 block text-xs text-slate-500">دقیقه</span>
                      <select value={minute} onChange={(event) => setMinute(Number(event.target.value))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm">
                        {Array.from({ length: 12 }, (_, value) => value * 5).map((value) => <option key={value} value={value}>{toPersianDigits(String(value).padStart(2, "0"))}</option>)}
                      </select>
                    </label>
                  </div>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <button type="button" disabled={!canSchedule || loading} onClick={() => void createJob(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">
                    {publishing ? <Loader2 size={17} className="animate-spin" /> : <Send size={17} />} انتشار الآن
                  </button>
                  <button type="button" disabled={!canSchedule || loading} onClick={() => void createJob(false)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-800 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center text-sm text-slate-400">هنوز محتوایی ثبت نشده است.</div>
            ) : (
                ...
                {jobs.map((job) => (
                  <div key={job.id} className="rounded-xl border border-slate-200 p-3">
                    <div className="flex items-start gap-3">
                      {job.media[0] ? (
                        job.media[0].type === "IMAGE" ? <img src={job.media[0].publicUrl} alt={job.media[0].fileName || ""} className="h-16 w-16 shrink-0 rounded-lg object-cover" /> : <video src={job.media[0].publicUrl} className="h-16 w-16 shrink-0 rounded-lg object-cover" />
                      ) : <div className="h-16 w-16 shrink-0 rounded-lg bg-slate-100" />}
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-bold text-slate-800">{typeLabels[job.type]}</span>
                          <span className="text-xs text-slate-500">{statusLabels[job.status] || job.status}</span>
                        </div>
                        <p className="mt-1 truncate text-xs text-slate-500">@{job.instagramAccount?.igUsername || "Instagram"}</p>
                        <p className="mt-1 text-xs text-slate-400">{job.status === "PUBLISHED" ? formatDate(job.publishedAt) : formatDate(job.scheduledAt)}</p>
                      </div>
                    </div>

                    {job.errorMessage && <div className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{job.errorMessage}</div>}

                    <div className="mt-3 flex gap-2">
                      {job.status === "FAILED" && <button type="button" onClick={() => void retryJob(job.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 hover:bg-slate-50"><RefreshCw size={13} /> تلاش مجدد</button>}
                      {job.status !== "PUBLISHED" && job.status !== "CANCELLED" && <button type="button" onClick={() => void cancelJob(job.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 px-3 py-2 text-xs text-red-600 hover:bg-red-50"><Trash2 size={13} /> لغو</button>}
                    </div>
                  </div>
                ))}
              </div>
