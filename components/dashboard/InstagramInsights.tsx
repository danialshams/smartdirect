      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs leading-6 text-slate-500">
            روند دسترسی، بازدید و تعاملات پیج را بررسی کنید.
          </p>
        </div>
      ...
      {loading && !data ? (
        ...
        <div className="p-4 sm:p-5 lg:p-6">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <MetricCard
              label="دسترسی"
              value={number(summary.reach)}
              helper={"در " + range + " روز"}
              icon={Eye}
            />
            <MetricCard
              label="بازدید"
              value={number(summary.views)}
              helper={"در " + range + " روز"}
              icon={BarChart3}
            />
            <MetricCard
              label="تعامل"
              value={number(summary.totalInteractions)}
              helper={percent(summary.engagementRate)}
              icon={Heart}
            />
            <MetricCard
              label="دنبال‌کنندگان"
            ...
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  داده‌های ثبت‌شده در SmartDirect
                </p>
              </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <MiniStat
              ...
              value={number(summary.accountsEngaged)}
            />
            <MiniStat
              label="بازدید پروفایل"
              value={number(summary.profileViews)}
            />
            <MiniStat
              label="نرخ تعامل"
