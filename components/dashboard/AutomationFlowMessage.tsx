...
import { ArrowDown, ArrowUp, ChevronDown, Image as ImageIcon, MessageSquare, Plus, Trash2, Video, Volume2, X } from "lucide-react";
...
type AutomationFlowMessageProps = {
    message: MessageDraft; index: number; total: number;
    messageOptions: { id: string; label: string }[];
    showcases: Showcase[]; forms: FormItem[]; loadingResources: boolean;
    instagramAccountId?: string;
    onUpdate: (patch: Partial<MessageDraft>) => void; onRemove: () => void;
    onMoveUp: () => void; onMoveDown: () => void; onAddQuickReply: () => void;
    onUpdateQuickReply: (quickReplyId: string, patch: Partial<QuickReplyDraft>) => void;
    onRemoveQuickReply: (quickReplyId: string) => void;
    onShowcaseCreated?: (showcase: Showcase) => void; onFormCreated?: (form: FormItem) => void;
};
...
export default function AutomationFlowMessage({ message, index, total, messageOptions, showcases, forms, loadingResources, instagramAccountId, onUpdate, onRemove, onMoveUp, onMoveDown, onAddQuickReply, onUpdateQuickReply, onRemoveQuickReply, onShowcaseCreated, onFormCreated }: AutomationFlowMessageProps) {
    ...
    const [availableShowcases, setAvailableShowcases] = useState<Showcase[]>(showcases);
    const [availableForms, setAvailableForms] = useState<FormItem[]>(forms);

    useEffect(() => setAvailableShowcases(showcases), [showcases]);
    useEffect(() => setAvailableForms(forms), [forms]);
    useEffect(() => {
        const onCreated = (event: Event) => {
            const detail = (event as CustomEvent<{ kind: ResourceKind; resource: Showcase | FormItem }>).detail;
            if (detail?.kind === "SHOWCASE") setAvailableShowcases((current) => [detail.resource as Showcase, ...current.filter((item) => item.id !== detail.resource.id)]);
            if (detail?.kind === "FORM") setAvailableForms((current) => [detail.resource as FormItem, ...current.filter((item) => item.id !== detail.resource.id)]);
        };
        window.addEventListener("smartdirect-resource-created", onCreated);
        return () => window.removeEventListener("smartdirect-resource-created", onCreated);
    }, []);

    function openCreator(kind: ResourceKind) {
        setCreator(kind); setResourceError(""); setTitle(""); setDescription(""); setFields([emptyField()]); setItems([emptyItem()]);
    }
    ...
    async function resolveAccountId() {
        ...
        const ids = new Set(accounts.map((account: { id: string }) => account.id));
        const selectedFromPage = Array.from(document.querySelectorAll("select"))
            .map((element) => (element as HTMLSelectElement).value)
            .find((value) => ids.has(value));
        return selectedFromPage || accounts[0]?.id || "";
    }

    async function createResource() {
        try {
            const accountId = await resolveAccountId();
            if (!accountId) throw new Error("ابتدا یک اکانت Instagram انتخاب کنید.");
            if (!title.trim()) throw new Error(creator === "FORM" ? "عنوان فرم را وارد کنید." : "عنوان ویترین را وارد کنید.");
            setSavingResource(true); setResourceError("");
            const endpoint = creator === "FORM" ? "/api/forms" : "/api/showcases";
            const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ instagramAccountId: accountId, title: title.trim(), description: description.trim() || null, isActive: true }) });
            const result = await response.json();
            ...
            if (creator === "FORM") {
                for (let i = 0; i < fields.length; i += 1) {
                    ...
                    const fieldResponse = await fetch(`/api/forms/${created.id}/fields`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ label: field.label.trim(), name: field.name.trim(), type: field.type, required: field.required, placeholder: field.placeholder.trim() || null, options: needsOptions ? options : null, order: i }) });
                ...
                setAvailableForms((current) => [form, ...current.filter((item) => item.id !== form.id)]);
                window.dispatchEvent(new CustomEvent("smartdirect-resource-created", { detail: { kind: "FORM", resource: form } }));
                onFormCreated?.(form); onUpdate({ formId: created.id });
            } else {
                for (let i = 0; i < items.length; i += 1) {
                    const item = items[i];
                    if (!item.title.trim()) throw new Error(`عنوان آیتم ${i + 1} را وارد کنید.`);
                    const itemResponse = await fetch(`/api/showcases/${created.id}/items`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: item.title.trim(), description: item.description.trim() || null, imageUrl: item.imageUrl.trim() || null, price: item.price.trim() || null, originalPrice: item.originalPrice.trim() || null, linkUrl: item.linkUrl.trim() || null, buttonText: item.buttonText.trim() || null, order: i, isActive: true }) });
                ...
                setAvailableShowcases((current) => [showcase, ...current.filter((item) => item.id !== showcase.id)]);
                window.dispatchEvent(new CustomEvent("smartdirect-resource-created", { detail: { kind: "SHOWCASE", resource: showcase } }));
                onShowcaseCreated?.(showcase); onUpdate({ showcaseId: created.id });
            }
            setCreator(null);
        } catch (error) {
    ...
    return <div className="space-y-4">
        <div><label className="mb-2 block text-xs font-semibold text-slate-600">نوع پیام</label><div className="relative"><select value={message.messageType} onChange={(event) => onUpdate({ messageType: event.target.value as MessageDraft["messageType"], text: "", mediaUrl: "", mediaId: "", showcaseId: "", formId: "" })} className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 py-3 pl-10 text-sm text-slate-700 outline-none focus:border-slate-400"><option value="TEXT">متن</option><option value="IMAGE">تصویر</option><option value="VIDEO">ویدیو</option><option value="AUDIO">صوت</option><option value="SHOWCASE">ویترین</option><option value="FORM">فرم</option></select><ChevronDown size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /></div></div>
        ...
        {message.messageType === "SHOWCASE" && <div><label className="mb-2 block text-xs font-semibold text-slate-600">ویترین</label><div className="flex gap-2"><div className="relative flex-1"><select value={message.showcaseId} onChange={(event) => onUpdate({ showcaseId: event.target.value })} disabled={loadingResources} className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 py-3 pl-10 text-sm outline-none disabled:opacity-50"><option value="">{loadingResources ? "در حال دریافت..." : "انتخاب ویترین"}</option>{availableShowcases.map((showcase) => <option key={showcase.id} value={showcase.id}>{showcase.title}</option>)}</select><ChevronDown size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /></div><button type="button" onClick={() => openCreator("SHOWCASE")} className="shrink-0 rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50">+ ساخت ویترین</button></div>{!loadingResources && availableShowcases.length === 0 && <p className="mt-2 text-[11px] text-slate-400">ویترین فعالی وجود ندارد؛ می‌توانید همینجا ویترین بسازید.</p>}</div>}
        {message.messageType === "FORM" && <div><label className="mb-2 block text-xs font-semibold text-slate-600">فرم</label><div className="flex gap-2"><div className="relative flex-1"><select value={message.formId} onChange={(event) => onUpdate({ formId: event.target.value })} disabled={loadingResources} className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 py-3 pl-10 text-sm outline-none disabled:opacity-50"><option value="">{loadingResources ? "در حال دریافت..." : "انتخاب فرم"}</option>{availableForms.map((form) => <option key={form.id} value={form.id}>{form.title}</option>)}</select><ChevronDown size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /></div><button type="button" onClick={() => openCreator("FORM")} className="shrink-0 rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50">+ ساخت فرم</button></div>{!loadingResources && availableForms.length === 0 && <p className="mt-2 text-[11px] text-slate-400">فرمی وجود ندارد؛ می‌توانید همینجا فرم بسازید.</p>}</div>}

        {creator && <div className="rounded-2xl border border-slate-300 bg-slate-50 p-4"><div className="mb-4 flex items-center justify-between"><div><p className="text-sm font-bold text-slate-800">ساخت {creator === "FORM" ? "فرم" : "ویترین"}</p><p className="mt-1 text-[11px] text-slate-500">مورد جدید برای همین اکانت ساخته و بلافاصله به این پیام متصل می‌شود.</p></div><button type="button" onClick={() => setCreator(null)} className="rounded-lg p-1 text-slate-400 hover:bg-white"><X size={16} /></button></div><div className="space-y-3"><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={creator === "FORM" ? "عنوان فرم" : "عنوان ویترین"} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none" /><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={2} placeholder="توضیح اختیاری" className="w-full resize-none rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none" />
        ...
        <button type="button" onClick={() => void createResource()} disabled={savingResource} className="inline-flex w-full items-center justify-center rounded-xl bg-slate-950 px-4 py-3 text-xs font-semibold text-white disabled:opacity-50">{savingResource ? "در حال ساخت..." : `ساخت ${creator === "FORM" ? "فرم" : "ویترین"} و انتخاب آن`}</button></div></div>}

        <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-slate-700">Quick Replies</p><p className="mt-1 text-[10px] leading-5 text-slate-400">کاربر با انتخاب پاسخ سریع به پیام مقصد منتقل می‌شود.</p></div><span className="text-[10px] text-slate-400">{message.quickReplies.length}/13</span></div>{message.quickReplies.length > 0 && <div className="mt-4 space-y-3">{message.quickReplies.map((quickReply, qrIndex) => <div key={quickReply.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-semibold text-slate-500">پاسخ سریع {qrIndex + 1}</span><button type="button" onClick={() => onRemoveQuickReply(quickReply.id)} className="text-slate-400 hover:text-red-600"><X size={15} /></button></div><div className="space-y-3"><div><label className="mb-1.5 block text-[10px] font-semibold text-slate-500">عنوان</label><input value={quickReply.title} maxLength={20} onChange={(event) => onUpdateQuickReply(quickReply.id, { title: event.target.value })} placeholder="مثلاً: بله، نمایش بده" className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-xs outline-none" /><div className="mt-1 text-left text-[9px] text-slate-400">{quickReply.title.length}/20</div></div><div><label className="mb-1.5 block text-[10px] font-semibold text-slate-500">رفتن به</label><div className="relative"><select value={quickReply.nextMessageId ?? ""} onChange={(event) => onUpdateQuickReply(quickReply.id, { nextMessageId: event.target.value || null })} className="w-full appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 pl-8 text-xs outline-none"><option value="">انتخاب پیام مقصد</option>{messageOptions.filter((option) => option.id !== message.id).map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" /></div></div></div></div>)}</div>}{message.quickReplies.length < 13 && <button type="button" onClick={onAddQuickReply} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 px-3 py-2.5 text-xs font-medium text-slate-600 hover:bg-slate-50"><Plus size={14} /> افزودن Quick Reply</button>}</div>
        <div className="flex items-center justify-between gap-2 border-t border-slate-200 pt-3"><div className="flex items-center gap-1"><button type="button" disabled={index === 0} onClick={onMoveUp} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-400 disabled:opacity-30"><ArrowUp size={15} /></button><button type="button" disabled={index === total - 1} onClick={onMoveDown} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-400 disabled:opacity-30"><ArrowDown size={15} /></button></div><div className="flex items-center gap-2"><span className="hidden text-[10px] text-slate-400 sm:inline">{getMessageTypeLabel(message.messageType)}</span>{total > 1 && <button type="button" onClick={onRemove} className="flex items-center gap-1 rounded-lg border border-red-100 px-2.5 py-2 text-xs text-red-600 hover:bg-red-50"><Trash2 size={14} /> حذف</button>}</div></div>
    </div>;
}
