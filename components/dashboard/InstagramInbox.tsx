        ...
        <section id="messages" dir="rtl" className="scroll-mt-24 space-y-4">
            ...
            {error && (
                <div className="flex items-center justify-between gap-4 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">
                    <span>{error}</span>
                    <button type="button" onClick={() => setError("")} className="font-medium hover:underline">بستن</button>
                </div>
            )}
                    ...
                    <div className="max-h-[550px] overflow-y-auto">
                        {loading && !conversations.length ? (
                            ...
                            filteredConversations.map((conversation) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full border-b border-slate-100 px-4 py-4 text-right transition ${active ? "bg-white" : "hover:bg-white"}`}
                                    >
                            ...
                            <div className="px-5 py-14 text-center">
                                <MessageCircle size={25} className="mx-auto text-slate-300" />
                                <p className="mt-3 text-sm font-medium text-slate-500">هنوز گفتگویی ثبت نشده است</p>
                                <p className="mt-1 text-xs leading-5 text-slate-400">بعد از دریافت اولین DM، گفتگو اینجا نمایش داده می‌شود.</p>
                            </div>
                        )}
                    </div>
                </aside>
                ...
                <div className="flex min-h-[620px] min-w-0 flex-col">
                    {selectedConversation ? (
                            ...
                            <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-6">
                                <div className="flex min-w-0 items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <h3 className="truncate text-sm font-bold text-slate-900">گفتگو با کاربر Instagram</h3>
                                        <p className="mt-1 truncate text-[10px] text-slate-400">{selectedConversation.participantId}</p>
                                    </div>
                                </div>
                                <span className={`rounded-full px-2.5 py-1 text-[10px] font-medium ${selectedConversation.isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                                    {selectedConversation.isActive ? "فعال" : "غیرفعال"}
                                </span>
                            </header>
                                    ...
                                    messages.map((message) => (
                                        <div key={message.id} className={`flex ${message.direction === "OUTBOUND" ? "justify-start" : "justify-end"}`}>
                                            <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${message.direction === "OUTBOUND" ? "rounded-bl-md bg-slate-950 text-white" : "rounded-br-md border border-slate-200 bg-white text-slate-800"}`}>
                                                ...
                                                <p className={`mt-2 text-[9px] ${message.direction === "OUTBOUND" ? "text-slate-400" : "text-slate-400"}`}>
                            ...
                            <form onSubmit={sendMessage} className="border-t border-slate-200 bg-white p-4 sm:p-5">
                                <div className="flex items-end gap-3">
                                    <textarea
                                        ...
                                        className="min-h-[58px] flex-1 resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-800 outline-none placeholder:text-slate-400 focus:border-slate-300 focus:bg-white"
                                    />
                                    <button
                                        type="submit"
                                        disabled={!text.trim() || sending}
                                        className="inline-flex h-[58px] shrink-0 items-center gap-2 rounded-2xl bg-slate-950 px-5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
                                    >
                                        <Send size={16} />
                                        ارسال
                                    </button>
                        ...
                        <div className="flex flex-1 items-center justify-center p-8 text-center">
                            <div>
                                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-slate-500"><MessageCircle size={24} /></div>
                                <h3 className="mt-4 font-bold text-slate-800">یک گفتگو را انتخاب کنید</h3>
                                <p className="mt-2 max-w-sm text-sm leading-6 text-slate-400">از ستون سمت راست یک conversation را انتخاب کنید تا تاریخچه پیام‌ها و امکان پاسخ نمایش داده شود.</p>
                            </div>
                        </div>
                    )}
                </div>
