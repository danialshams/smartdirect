...
function ChangeBadge({ value }: { value: number | null }) {
  ...
  return (
    <span className={positive ? "inline-flex items-center gap-1 text-xs font-semibold text-emerald-600" : "inline-flex items-center gap-1 text-xs font-semibold text-red-600"}>
...
export default function AdvancedAnalyticsReports({ accountId: externalAccountId }: { accountId?: string }) {
  ...
  const [from, setFrom] = useState(inputDate(new Date(today.getTime() - 29 * 86400000)));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/instagram/insights/history?days=90&accountId=" + encodeURIComponent(accountId),
        { cache: "no-store" },
      );
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || "خطا در دریافت داده‌های گزارش");
      setData(result as Data);
    } catch (requestError) {
      setData(null);
      setError(requestError instanceof Error ? requestError.message : "خطا در دریافت داده‌های گزارش");
    } finally {
      setLoading(false);
    }
  }, [accountId]);
  ...
  const report = useMemo(() => {
    if (!data || !from || !to || dayStart(from) > dayEnd(to)) return null;
    ...
    const currentMetrics = Object.fromEntries(metrics.map(({ key }) => [key, sum(current, key)])) as Record<MetricKey, number>;
    const previousMetrics = Object.fromEntries(metrics.map(({ key }) => [key, sum(previous, key)])) as Record<MetricKey, number>;

    return { current, previous, currentMetrics, previousMetrics };
  }, [data, from, to]);
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <div className="border-b border-slate-100 px-5 py-5 sm:px-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-lg font-bold tracking-tight text-slate-950">گزارش عملکرد پیج</h2>
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex">
            ...
            <button
              ...
              onClick={exportCsv}
              disabled={!report}
              className="col-span-2 inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 sm:col-span-1"
            >
      ...
      {loading && !data ? (
        ...
        <div className="p-4 sm:p-5 lg:p-6">
            ...
            {metrics.map(({ key, label }) => {
              ...
              return (
                <div key={key} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-xl font-bold tracking-tight text-slate-950">{number(current)}</p>
                  <div className="mt-2 flex items-center justify-between gap-2">
                    <span className="text-[10px] text-slate-400">دوره قبل: {number(previous)}</span>
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-slate-200">
            <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-4">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-950">جزئیات بازه</h3>
                <p className="mt-0.5 text-[10px] text-slate-400">{report.current.length} روز داده در بازه انتخابی</p>
              </div>
            </div>

            {report.current.length === 0 ? (
              <div className="px-5 py-12 text-center text-sm text-slate-400">برای این بازه داده‌ای ثبت نشده است.</div>
            ) : (
                ...
                <table className="w-full min-w-[800px] text-right text-xs">
                  <thead className="bg-slate-50 text-slate-400">
                    <tr>
                      ...
                      {metrics.map((metric) => (
                        <th key={metric.key} className="px-4 py-3 font-medium">{metric.label}</th>
                      ))}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {[...report.current].reverse().map((snapshot) => (
                      <tr key={snapshot.id} className="text-slate-700">
                        <td className="px-4 py-3 font-medium text-slate-800">{date(snapshot.snapshotDate)}</td>
                        {metrics.map((metric) => (
                          <td key={metric.key} className="px-4 py-3">{number(snapshot[metric.key])}</td>
                        ))}
                        <td className="px-4 py-3">{number(snapshot.followerCount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="px-5 py-16 text-center text-sm text-slate-400">برای ساخت گزارش، یک پیج و بازه زمانی انتخاب کنید.</div>
      )}
    </section>
  );
}
